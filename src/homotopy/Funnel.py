...
class Funnel():
    def __init__(self, surface, start, target, similar_path, vtx_map, holes=None):
        ...
        self.fan = [
            vtx_map[similar_path[0][0]], start, vtx_map[similar_path[0][1]]
        ]
        self.tail = [start]
        self.apex = start
    ...
    def funnel(self, visualize=True):
        ...
        for cnt in range(len(self.similar_path) - 1):
            ...
            if visualize:
                fv.plot_funnel(self.similar_path, self.fan, self.tail,
                               self.apex, edge, self.start, self.target)
            ...
            if np.sum(self.fan[0] - pt1) == 0:
                ...
                raise ValueError(
                    "pt1 and pt2 are in neither of the boundary arrays")
    ...
    def add_to_boundary(self, pt, badpt, side, edge):
        ...
        if side == 'left':
            ...
            ray_origin = ray[0]
        intersect = self.line_ray_intersect(ray_origin, ray_direction, edge[0],
                                            edge[1], badpt)
    ...
    def line_ray_intersect(self, rayOrigin, rayDirection, point1, point2,
                           badpt):
